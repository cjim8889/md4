...
def get_config() -> config_dict.ConfigDict:
    ...
    config.vocab_size = 3000  # Vocabulary size for tokenizer - determines embedding table size
    config.dataset = "pubchem_large_text"  # Dataset type - controls which input pipeline is used
    config.version = "1.1.0"  # Dataset version for reproducibility and data tracking
    config.training_shards = 128  # Number of training data shards for parallel processing
    config.validation_shards = 4  # Number of validation data shards
    ...
    config.interior_frac = 0.2  # Fraction of padding tokens placed in sequence interior vs edges
    ...
    config.record_shuffle_buffer = 10000   # Buffer size for shuffling records within files (training)
    ...
    config.max_length = 128  # Maximum sequence length for input tokens
    config.tokenizer = "data/sentencepiece_tokenizer_bpe_3000_newcorpus.model"  # Path to tokenizer model
    ...
    config.timesteps = 500  # Number of diffusion timesteps for noise schedule
    ...
    config.n_layers = 20  # Number of transformer layers - controls model depth (larger model)
    config.dropout_rate = 0.0  # Dropout probability for regularization (0.0 = no dropout)
    config.multiple_of = 32  # Constraint to make layer dimensions multiples of this for efficiency
    ...
    config.cross_attention_layers = 8  # First N layers use cross-attention (None = all layers)
    config.cross_attention_proj_dim = 256  # Projection dimension for cross-attention mechanism
    config.cross_conditioning_seq_length = 16  # Sequence length for reshaping cross-conditioning data
    ...
    config.scale_by_muon = False  # Whether to use Muon optimizer scaling (disabled for stability)
    ...
    config.num_train_steps = 600_000  # Total number of training steps (longer training)
    config.num_eval_steps = 1000  # Number of evaluation steps per eval round (-1 = full epoch)
    config.batch_size = 8192  # Global batch size across all devices (larger than safe_split config)
    config.num_microbatches = 4  # Number of gradient accumulation steps for memory efficiency
    ...
    # Checkpoint directory configuration - GCS bucket for persistent storage
