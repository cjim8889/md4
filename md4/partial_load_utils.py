...
def should_use_partial_loading(config) -> bool:
    ...
    """
    return (hasattr(config, 'old_config') and 
            config.old_config is not None and 
            'molecular.py' in str(config.old_config))
...
def partial_load_checkpoint(
    ...
    try:
        ...
        new_ema_params = dict(train_state.ema_params) if hasattr(train_state, 'ema_params') and train_state.ema_params is not None else None
        ...
        if new_params_only:
            logging.info(f"New parameters (kept from fresh initialization): {new_params_only}")
